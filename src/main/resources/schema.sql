...
CREATE TABLE IF NOT EXISTS inspection_query (
    ...
    query_sql             CLOB,
    query_description_zh  CLOB,
    query_description_en  CLOB,
    enabled               INT          DEFAULT 1,
    sort_order            INT          DEFAULT 0,
    created_at            TIMESTAMP    DEFAULT CURRENT_TIMESTAMP,
    updated_at            TIMESTAMP    DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_q_ch FOREIGN KEY (chapter_id)
        REFERENCES inspection_chapter(id) ON DELETE CASCADE,
    CONSTRAINT uq_insp_q UNIQUE (chapter_id, query_key)
);
...
CREATE INDEX IF NOT EXISTS idx_insp_q_ch       ON inspection_query(chapter_id);
