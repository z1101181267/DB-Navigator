...
Web 控制台共 **14 个入口，按三组收纳**：

| 分组 | 入口 |
| --- | --- |
| **大屏展示** | 概览 |
| **数据库运维** | 数据库管理 · 数据库统计 · 定时巡检 · 巡检执行 · 巡检历史 · SQL 编辑器 |
| **配置与管理** | 数据源纳管 · 驱动管理 · 巡检配置 · 基线配置管理 · 规则引擎 · 插件市场 · AI配置 |

其中「巡检历史」支持报告**在线预览**与 **Word / PDF / HTML 下载**。

> **哪些是骨架页**：「数据库管理」的库/表/字段结构浏览、「定时巡检」的调度器、
> 「插件市场」的插件源、「AI配置」的模型服务当前**尚未接入后端**，页面里都写明了缺口与
> 待接入的接口，不会假装能用。能接真实数据的一律接真实数据（概览、数据库统计全部取自
> 现有接口）。详见 [§10 骨架页与能力边界](#骨架页与能力边界)。
...
分为三组，与界面上「巡检配置 / 基线配置管理 / 规则引擎」三个菜单对应。
...
| `scripts/dom_smoke.js` | 在 jsdom 中真实执行 `app.js`，验证侧栏三分组、14 个视图切换、渲染、页签、弹窗、驱动两栏布局、巡检执行与巡检历史交互，以及 6 个新页面的结构与边界声明 | `NODE_PATH=<node_modules> node scripts/dom_smoke.js` |
| `scripts/ui_guard.js` | **真实浏览器**（headless Edge/Chrome + CDP）验证元素显隐、视图互斥、侧栏分组折叠与几何、条形图真实宽度、禁用态、条件字段联动、两栏真实几何、iframe 预览加载、按钮配色 | `node scripts/ui_guard.js` |
...
两栏是否等高、表头是否真的 `position: sticky`、侧栏 13 个入口是否把底部区块挤出视口。
这些只有真实浏览器能回答——jsdom 的 `getComputedStyle` 对布局属性给不出可信结果。

第三类是**「静默塌成 0」**——最容易被漏掉的一类。条形图的 `.bar-fill` 是个 `<span>`，
而 `<span>` 默认 `display: inline`，`height: 100%` 对它完全无效，条会静默地不显示任何长度；
只有把 `.bar-fill` 提成块级元素才撑得出来。这类问题在 DOM 里查不出（`style="width:60%"` 看着一切正常），
必须量真实像素宽度。同理，「禁用按钮」如果没写 `.btn:disabled` 样式，
实心的 `.btn-primary` 在禁用时看着仍然可点 —— 也靠真浏览器断言 `opacity` 与 `cursor` 兜住。

```bash
node scripts/ui_guard.js                                    # 69 项（默认打 8080）
DBNAV_BASE=http://127.0.0.1:9090 node scripts/ui_guard.js   # 69 项
```
...
node scripts/dom_smoke.js                            # 137 项（默认打 8080）
DBNAV_BASE=http://127.0.0.1:9090 node scripts/dom_smoke.js   # 137 项
node scripts/ui_guard.js                             # 69 项（默认打 8080）
DBNAV_BASE=http://127.0.0.1:9090 DBNAV_CDP_PORT=9334 node scripts/ui_guard.js   # 69 项
```
...
导航为 **14 个入口、三个分组**（分组标题可点击收起；切换视图时会自动展开目标所在分组，
否则高亮项藏在收起的分组里，点了菜单会像没反应）：

**大屏展示**

| 模块 | 能力 |
|---|---|
| **概览** | 一屏看清平台全貌：6 张 KPI 卡（纳管数据源 / 可用驱动 / 巡检模板 / 规则库 / 基线规则 / 执行次数）、各库类型巡检资产条形图、基线风险四色分布、最近 5 次执行、常用入口快捷跳转 |

**数据库运维**

| 模块 | 能力 |
|---|---|
| **数据库管理** | **运维视角**，与「配置与管理 › 数据源纳管」的登记视角分开。左栏列已纳管实例，右栏展示实例运行信息（地址 / 库名 / 连接状态 / 最近连通 / 驱动版本）并可**测试连接**（回显耗时、驱动版本与产品名），附「去 SQL 编辑器」「改连接参数」快捷入口。库 / 表 / 字段的结构浏览**尚未接入**，页面写明缺口与待接入接口 |
| **数据库统计** | 6 张 KPI + 5 张分布图：按库类型的规则与章节分布、基线覆盖、规则启用 / 停用 / 未被引用占比、规则归类 Top 10、按日的执行次数与平均合规率 |
| **定时巡检** | 按周期自动发起巡检。调度器**尚未接入**：任务表保持空态、新建按钮禁用，但「执行记录的触发方式」用的是真实数据（当前全部为手动触发）—— 用事实说明缺口在哪，而不是只写一句「敬请期待」 |
...
| **巡检配置** | 报告骨架。两个页签：**巡检模板**（模板列表 → 章节手风琴 → 该章引用的规则，章节可增删改；「引用规则」弹窗从规则库里挑同库类型的规则绑定 / 解绑）、**变更历史**（增删改留痕）。规则正文不在这里编辑，见「规则引擎」 |
...
| **插件市场** | 扩展平台能力（巡检规则包 / 驱动包 / 报告模板）。插件源**尚未接入**：搜索与分类控件就位但禁用，列表为空，且**不预置任何「示例插件」** —— 免得被当成真能装。要加 JDBC 驱动请走「驱动管理」的上传链路，那条是通的 |
| **AI配置** | 把巡检结果交给大模型做归因与处置建议。模型服务**尚未接入**：表单按接入后的形态摆好，但「保存」与「测试连接」禁用，且当前不读写任何凭据 |

### 骨架页与能力边界

「数据库管理」的对象浏览、「定时巡检」的调度器、「插件市场」的插件源、「AI配置」的模型服务，
这四项后端能力尚未接入。处理方式统一为三条：

1. **能接真实数据的就接真实数据** —— 概览与数据库统计的每一个数字都来自现有接口，
   没有一处是写死的（`/summary`、`/rules/stats`、`/runs`、`/datasources`、`/drivers`）。
2. **把缺口写在页面上** —— 统一的警示色「能力边界」块，列出待接入的接口与它解决什么问题。
3. **不伪造成功** —— 保存类按钮禁用（`.btn:disabled` 有可见的禁用态与 `not-allowed` 光标），
   列表保持空态，不预置「示例插件」「示例任务」这类看着像真能用的占位数据。

理由：骨架页最大的风险不是「功能少」，而是「看起来能用」。按钮点下去弹出「保存成功」，
用户就会相信配置生效了，而它其实什么也没发生 —— 这种错误的代价远高于页面空着。
...
│   ├── dom_smoke.js                      # jsdom 前端行为冒烟测试（137 项，不跑 CSS）
│   ├── ui_guard.js                       # 真实浏览器 UI 守卫（69 项，headless Edge + CDP，零依赖）
...
| 导航结构 | 14 个入口按**大屏展示 / 数据库运维 / 配置与管理**三组收纳，分组可收起 | 平铺 14 项时「改配置」和「看结果」混在一起，日常运维每次都要在长列表里找；分组按使用动线排（先看整体 → 再做运维 → 最后才改配置），越靠下的越不常动 |
| 导航结构 | 驱动管理 / 基线配置管理 / 规则引擎 / 巡检历史各为**独立入口** | 基线配置与「发起巡检」是两件事，塞进同一个页签会让日常操作变绕；规则库是跨模板复用的资产，挂在某个模板下面就没法表达「一条规则被多个模板引用」 |
| 数据库管理 vs 数据源纳管 | 前者是**运维视角**（看实例状态、测连接、浏览对象），后者是**登记视角**（增删改连接参数） | 两个动作的使用频率与风险不同：日常运维只读，改连接参数是低频且要谨慎的操作，混在一页容易误改 |
| 规则归属 | 规则正文集中在规则库，章节只存引用关系 | 同一段 SQL 在多个模板里各存一份，改一处就得记得改完其余几处 |
| 未接入的能力 | 页面照建，缺口写在页面上，保存类按钮禁用 | 骨架页最大的风险是「看起来能用」；按钮弹「保存成功」而实际什么也没发生，代价远高于页面空着 |
